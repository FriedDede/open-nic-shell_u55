...
def read_bin():
    ...
    with open(filename + ".img", 'rb') as f:
        rom = bytearray(reversed(f.read()))
        st_rom = (binascii.hexlify(rom))
        print(st_rom)
    # align to 64 bit
...
with open(filename + ".sv", "w") as f:
    ...
    for i in (range(int(len(rom)/16))):
        up_str = str(rom[i*16:i*16+8])
        up_str = up_str[+2:-1]
        low_str = str(rom[i*16+8:i*16+16])
        low_str = low_str[+2:-1]
        rom_str += "        64'h" + up_str + "_" + low_str + ",\n"
